...
Feature: centralized scheduled bc-base dependency check-bump-rebuild (lead-czwo)
  ...
  @scenario_hash:ed39191f3158079e @bc:shopsystem-bc-launcher
  Scenario: the poll resolves each baked dependency's latest as the semver-max release and only bumps on a strictly-forward version, never downgrading and never dying on one bad release
    Given the bc-base Dockerfile in shopsystem-bc-launcher pins the baked dependency "shop-templates" at "v0.52.0"
    ...
    And when the resolved latest for "shop-templates" is "v0.45.0" while the pin is "v0.52.0", the executable body treats the behind-or-equal result as a no-op: it does not rewrite the pin to the lower "v0.45.0" and does not exit non-zero
    ...
    And an executable body that rewrote the "shop-templates" pin from "v0.52.0" down to "v0.45.0" or exited non-zero on that behind-resolution would not satisfy this behavior
